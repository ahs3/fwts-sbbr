...
static void acpi_table_check_fadt_sci_int(fwts_framework *fw)
{
	// TODO: is there any way to verify the SCI interrupt
	// on an x86 system?  The SCI_INT field should contain a GSI
	// to represent the SCI and it must be a sharable, level,
	// active low interrupt.
	//
	// This may not be possible, in which case this routine should
	// go away.
	fwts_log_info(fw, "FADT SCI_INT is %d", fadt->sci_int);
	return;
}
...
static int fadt_test1(fwts_framework *fw)
{
	...
	if (!fwts_acpi_is_reduced_hardware(fadt)) {
		acpi_table_check_fadt_sci_int(fw);
		...
		/* cannot test gpe1_base since any value is reasonable */
		acpi_table_check_fadt_cst_cnt(fw);

		/*
		 * TODO: test p_lvl2_lat, which must be <= 100 iff the
		 * system supports a C2 state.  If there is no C2 state,
		 * it must be > 100.  Also, see comments below.
		 */

		/*
		 * TODO: test p_lvl3_lat, which must be <= 1000 iff the
		 * system supports a C3 state.  If there is no C3 state,
		 * it must be > 1000.  Also, see comments below.
		 */

		/* cannot test flush_size since any value is reasonable */
		/* cannot test flush_stride since any value is reasonable */
		/* cannot test duty_offset since any value is reasonable */
		/* cannot test duty_width since any value is reasonable */
		/* cannot test day_alrm since any value is reasonable */
		/* cannot test mon_alrm since any value is reasonable */
		/* cannot test century since any value is reasonable */

		/*
		 * TODO: test sleep_control_reg; is there anything to go
		 * on, however?
		 */

		/*
		 * TODO: test sleep_status_reg; is there anything to go
		 * on, however?
		 */

		/*
		 * Cannot test Hypervisor Vendor Identity since value is
		 * provided by hypervisor vendors and not controllable.
		 */
	}
